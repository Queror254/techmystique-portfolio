...
const getTitle = (viewMode: ViewMode, activeCommand: string, activeGame: string | null) => {
    if (viewMode === 'game') {
    ...
    switch(activeCommand) {
        case 'init': fileName = 'welcome.sh'; break;
        case 'about': fileName = 'about.md'; break;
        case 'projects': fileName = 'projects.json'; break;
        case 'skills': fileName = 'skills.ts'; break;
        case 'contact': fileName = 'contact.html'; break;
        case 'help': fileName = 'help.txt'; break;
        case 'history': fileName = 'history.log'; break;
        case 'cowsay': fileName = 'cow.txt'; break;
        case 'run': fileName = 'runner.js'; break;
        case 'theme': fileName = 'settings.json'; break;
        case 'view': fileName = 'settings.json'; break;
        case 'ls': fileName = 'help.txt'; break;
        case 'ask': fileName = 'ask.ai'; break;
        case 'play': fileName = 'launcher.sh'; break;
    }
    return `${fileName} — ${YOUR_NAME} — Portfolio`;
}

const TopBar: React.FC<TopBarProps> = ({ activeCommand, isTerminalMode, onToggle, onToggleSidebar, activeGame, viewMode }) => {
  const title = getTitle(viewMode, activeCommand, activeGame);

  return (
    <header className="bg-[var(--bg-secondary)] text-[var(--text-primary)] flex items-center justify-between h-10 border-b border-[var(--border-primary)] px-2 md:px-4 flex-shrink-0 z-20">
